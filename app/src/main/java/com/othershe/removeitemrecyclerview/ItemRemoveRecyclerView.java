...
public class ItemRemoveRecyclerView extends RecyclerView {
    ...
    //删除按钮
    private TextView mDelete, mSetKnow;
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mDeleteBtnState == 0) {
                    View view = findChildViewUnder(x, y);//得到触摸点对应的Item View
                    if (view == null) {
                        return false;
                    }
                    ...
                    mSetKnow = (TextView) mItemLayout.findViewById(R.id.item_setKnow);
                    /**mMaxLength:这里是两按钮的宽度用于滑动删除，item向左滑出的距离*/
                    mMaxLength = mDelete.getWidth() + mSetKnow.getWidth();
                    ...
                    mScroller.startScroll(mItemLayout.getScrollX(), 0, -mMaxLength, 0, 200);//使Item自动右滑直到删除按钮完全隐藏
                    invalidate();
                    mDeleteBtnState = 0;
                    return false;
                } else {
